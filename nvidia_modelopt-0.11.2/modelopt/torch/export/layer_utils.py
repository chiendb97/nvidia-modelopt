...
def is_decoder_list(module: nn.Module) -> bool:
    ...
    return type(module) == nn.ModuleList and type(module[0]) != nn.Sequential
...
def is_medusa_head(module: nn.Module) -> bool:
    """Returns whether the module is an Medusa Head layer."""
    return type(module) == nn.ModuleList and type(module[0]) == nn.Sequential
...
def build_medusa_layer_config(module: nn.Module, dtype: torch.dtype) -> MedusaLayerConfig:
    config = MedusaLayerConfig()
    if module.act == nn.SiLU:
        config.hidden_act = "silu"
    else:
        config.hidden_act = "silu"
    config.linear = build_linear_config(module.linear, LINEAR_COLUMN, dtype)
    return config


def build_medusa_head_config(module: nn.Module, dtype: torch.dtype) -> MedusaHeadConfig:
    config = MedusaHeadConfig()
    children = list(module.children())
    medusa_layers = []
    for medusa_layer in children[:-1]:
        medusa_layers.append(build_medusa_layer_config(medusa_layer, dtype))
    config.medusa_layers = medusa_layers
    config.lm_head = build_linear_config(children[-1], LINEAR_COLUMN, dtype)
    return config
